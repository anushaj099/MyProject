         ...
         //   pc.setInvTot(iw);
            ...
            for (int i = 0; i < l1.size(); i++) {
               ...
               // System.out.println("------------------" + l2.get(i) + ")))))))))))" + l3.get(i) + "**" + l4.get(i));
                ...
                if((!"Frieght Charges".equals(l1.get(i)))||(!"FRIEGHT CHARGES".equals(l1.get(i)))||(!"Mining Charges".equals(l1.get(i)))||(!"MINING CHARGES".equals(l1.get(i)))){
                if(i==0)
                cc.setQty(l3.get(i));
                }
//                if(l3.get(i)!=null)
//                cc.setQty(l3.get(i));
   ...
   public void insertTaxInvoice(TaxInvoiceDto tdto,int lid) {
        System.out.println("the invoice innnsertionnnnn");
       Session s = HibernateUtil.getSessionFactory().openSession();
        Transaction t = s.beginTransaction();
        System.out.println("****************"+tdto.getBillToName()+"**********"+tdto.getBillToAddress());
...
//           q.executeUpdate();
    ...
    public List getGstno() {
         Session s = HibernateUtil.getSessionFactory().openSession();
        Transaction t = s.beginTransaction();
        List<Gst> list = new ArrayList<>();
      
        try {
            Criteria c = s.createCriteria(Gst.class);
           //c.setProjection(Projections.property("gst_number"));
            list = c.list();
        } catch (Exception e) {
            System.out.println("the error in retrieving grades in taximanager is------>"+e);
       ...
       System.out.println("gstttt&%%%%% list is:------->>>>" + list);
        return list;
   }
    ...
    public List getInvoice(){
        ...
        try {
             Session s = HibernateUtil.getSessionFactory().openSession();
        String hql="select invoiceNo from PrTaxIntra";
        Query q=s.createQuery(hql);
        l=q.list();
            System.out.println("lookkkkkkkkkk"+l);
        }
        catch(Exception e) {
            
        }
         System.out.println("invvvvnoooooo::::&&&&&&::"+l);
        return l;
    }
    
    public List invoiceEdit(int inv){
        List l=null;
        int id=0;
        try{
            Session s = HibernateUtil.getSessionFactory().openSession();
             ...
             Criteria c=s.createCriteria(PrTaxIntra.class);
             l=c.list();
             System.out.println("lllllllllll*******"+l);
             for(Object o:l){
                 PrTaxIntra pp=(PrTaxIntra)o;
                 id=pp.getInvoiceNo();
                 System.out.println("allllll::"+id);
                 if(id!=inv){
                     l.add("00"+id+"");
	       ...
	       //   PdfWriter.getInstance(document, file);
			...
			//Inserting Image in PDF
                          //   Image image = Image.getInstance ("E://Java//VSK_SC//SSK_Logo.png");
                              Image image = Image.getInstance ("D://SSK_Logo.png");
			//     Image image = Image.getInstance ("E://Java//SVK_RMC//svkl2.jpg");
			     image.scaleAbsolute(120f, 90f);//image width,height	
                             ...
                             // Image image1 = Image.getInstance ("E://Java//VSK_SC//vsk-head1.jpg");
                              Image image1 = Image.getInstance ("D://vsk-head1.jpg");
...
//                         
                           ...
                           //  p.add(new Chunk(image1, 0, 0));
                             ...
                             p.add(new Chunk(Chunk.NEWLINE+"VENKATA SAI KRISHNA STONE CRUSHER",boldFont));
                             p.add(new Chunk(Chunk.NEWLINE+"GSTIN:37AAGFV7908J1ZM",boldFont2));
                             p.add(new Chunk(Chunk.NEWLINE));
                           //  p.add(new Chunk(Chunk.NEWLINE+"\n Plant: Survey No. 398/A1, PERECHERLA, Guntur Dist. (A.P). ",regular));
                             p.add(new Chunk(Chunk.NEWLINE+"Works and Regd. Off:"+" 398/A1 A2,Narasaraopet Road,Opposite Dokiparru ",new Font(Font.FontFamily.TIMES_ROMAN,9,Font.NORMAL)));
...
//    p.add(new Chunk(Chunk.NEWLINE+"\n ",new Font(Font.FontFamily.TIMES_ROMAN,9,Font.NORMAL)));
                             p.add(new Chunk(Chunk.NEWLINE+"Mobile No: 9849913085, e-mail: vsksc999@gmail.com",new Font(Font.FontFamily.TIMES_ROMAN,9,Font.NORMAL)));
...
,normalfont2));
                    ...
                    cel10=new PdfPCell(new Phrase("For Venkata Sai Krishna\n" +
