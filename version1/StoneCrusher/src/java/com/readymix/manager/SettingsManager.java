...
public class SettingsManager {
      ...
      public List<Login> dataRetrieve(int id) {
       ...
       //      float f1=Float.valueOf(rate);
         ...
         public gstdto insertGst(gstdto form){
         Session s=HibernateUtil.getSessionFactory().openSession();
         Transaction t=s.beginTransaction();
         Gst l=new Gst();
         System.out.println("startttttttttt");
          String grade1=form.getGstNumber();
       //   String rate=form.getRate();
       ...
       //      float f1=Float.valueOf(rate);
        ...
        try {
           ...
           Gst pq=new Gst();
            Criteria cr = s.createCriteria(Gst.class);
            pq = (Gst) cr.add(Restrictions.eq("gstid", id)).uniqueResult();
            s.delete(pq);
            t.commit();
        ...
        try {
           Criteria c = s.createCriteria(Gst.class);
            list = c.list();
            System.out.println("gstttttttttlist issssss"+list);
        } catch (Exception e) {
            e.printStackTrace();
        }
        System.out.println("list is:------->>>>" + list);
...
//        query.setParameter("mobileNo", number);
        ...
        Criteria c=session.createCriteria(Gst.class);
        c.add(Restrictions.eq("gstNumber",number));
        List<Gst> list = c.list();
         System.out.println("????????????");
