...
public class ProductionManager {
    ...
    public void insertProduction(PtProductionDto ptp, String date, String superv, String mixingNumber, int issueNumber) throws ParseException {
        Session s = HibernateUtil.getSessionFactory().openSession();
        Transaction t = s.beginTransaction();

        ParentGdProduction pg = new ParentGdProduction();
        String dat = date;
        ...
        Criteria c = s.createCriteria(ParentGdProduction.class);
        pg = (ParentGdProduction) c.add(Restrictions.eq("date", date1)).add(Restrictions.eq("superviser", superv)).add(Restrictions.eq("mixingNumber", mixingNumber)).add(Restrictions.eq("issueNumber", issueNumber)).uniqueResult();
        s.close();
        ...
        for (int i = 0; i < l1.size(); i++) {
            ChildGdProduction cg = new ChildGdProduction();
            cg.setParentGdProduction(pg);
    ...
    public List productionList(PtProductionDto ptp) {
        Session s = HibernateUtil.getSessionFactory().openSession();
        Transaction t = s.beginTransaction();
    ...
    public List<ParentGdProduction> displayProduction() {
       Session s = HibernateUtil.getSessionFactory().openSession();
        Transaction t = s.beginTransaction();
    ...
    public boolean productionInsVal(PtProductionDto form) {
        Session s = HibernateUtil.getSessionFactory().openSession();
        Transaction t = s.beginTransaction();
        boolean p = false;
        try {
            ...
            l = c.list();
            for (Object o : l) {
                ParentGdProduction pd = (ParentGdProduction) o;
        ...
        if (!p) {
            ParentGdProduction pd = new ParentGdProduction();
        }
        return p;
    }

    public List packingvallist(PtProductionDto form) {
        Session s = HibernateUtil.getSessionFactory().openSession();
        Transaction t = s.beginTransaction();
        List l = null;
        try {
    ...
    public List productionEdit(int id) {
        Session s = HibernateUtil.getSessionFactory().openSession();
        Transaction t = s.beginTransaction();
        List<ParentGdProduction> l = null;
        try {
            ParentGdProduction e = new ParentGdProduction();
            Criteria c = s.createCriteria(ParentGdProduction.class);
            c.add(Restrictions.eq("idpp", id)).uniqueResult();
            l = c.list();
            t.commit();
    ...
    public void productionUpdate(PtProductionDto ptp, int id1) throws ParseException {
        Session s = HibernateUtil.getSessionFactory().openSession();
        Transaction t = s.beginTransaction();
        try {

            ParentGdProduction pd = new ParentGdProduction();
            pd = (ParentGdProduction) s.get(ParentGdProduction.class, id1);
            ...
            Criteria c = s.createCriteria(ParentGdProduction.class);
            pd = (ParentGdProduction) c.add(Restrictions.eq("date", date1)).add(Restrictions.eq("superviser", ptp.getSuperviser())).add(Restrictions.eq("mixingNumber", ptp.getMixingNumber())).add(Restrictions.eq("issueNumber", ptp.getIssueNumber())).uniqueResult();
            s.close();

            Session s1 = HibernateUtil.getSessionFactory().openSession();
            Transaction t1 = s1.beginTransaction();
            List lis = null;
            Criteria c2 = s1.createCriteria(ChildGdProduction.class);
            System.out.println("kjdgh----------------------dhg+++++++++++++++--------------child");
            lis = c2.add(Restrictions.eq("parentGdProduction.idpp", id1)).list();
            if (lis != null) {
                System.out.println("kjdgh--------ggggggggggggggggggg--------------child");
                for (Object o : lis) {
                    System.out.println("kjdgh---------dhg+++++++++++++++");
                    ChildGdProduction cq = (ChildGdProduction) o;
                    s1.delete(cq);
                }
            }
            List<String> l1 = ptp.getVariety();
            ...
            List<Float> l4 = ptp.getTotalBales();
            for (int i = 0; i < l1.size(); i++) {
                ChildGdProduction cd = new ChildGdProduction();
                cd.setParentGdProduction(pd);
                System.out.println("--------------------child--------------" + l1.get(i));
    ...
    public List productionEdit1(int idpp) {
        ...
        List l1 = null;
        try {
            Criteria cr = s1.createCriteria(ChildGdProduction.class);
            cr.add(Restrictions.eq("parentGdProduction.idpp", idpp));
            l1 = cr.list();
    ...
    public void productionDelete(int id1) {
        ...
        try {
            Criteria c = s.createCriteria(ParentGdProduction.class);
            ParentGdProduction e = (ParentGdProduction) c.add(Restrictions.eq("idpp", id1)).uniqueResult();
            s.delete(e);
            t.commit();
    ...
    public boolean check(PtProductionDto gsd) throws ParseException {

        Session s = HibernateUtil.getSessionFactory().openSession();
        Transaction t = s.beginTransaction();
        boolean succ = true;
        try {
            List<ParentGdProduction> l = new ArrayList<>();
            ...
            Date d = null;
            for (Object o : l) {
                ParentGdProduction gd = (ParentGdProduction) o;
                d = gd.getDate();
            }
    ...
    public boolean validateData(PtProductionDto ptp) throws ParseException {
        Session s = HibernateUtil.getSessionFactory().openSession();
        Transaction t = s.beginTransaction();
        Criteria c = s.createCriteria(ParentGdProduction.class);
    ...
    public List shiftExists() {
        ...
        try {
            Criteria c = s.createCriteria(ParentGdProduction.class).setProjection(Projections.projectionList().add(Projections.max("date")));;
            l = c.list();
            System.out.println("//////////////" + l);
    ...
    public List getProductList() {
        ...
        String sql = "select product_name from products_sample where partial_type like '%Issue to Department%'";
